  ...
  try {
    ...
    for (const it of items) {
      ...
      const { rowCount } = await client.query(
        `INSERT INTO news (newsId, title, content, summary, url, pubTime, webPath, stock, stockCode, stockName, otherData)
    ...
    console.log(`mergeNewsFromUrl(${url}): inserted ${added}, ignored ${ignored}`);
...
if (timeCandidate) {
  ...
  if (pubDate && isNaN(pubDate.getTime())) {
    console.warn(`Invalid date for item ${it.newsId}: ${timeCandidate}`);
    pubDate = null;
  }
}
      ...
      if (rowCount > 0) {
        ...
        ignored++;
        console.log(`Ignored item with newsId: ${it.newsId} (possible duplicate)`);
      }
    }
    
    await client.query('COMMIT');
    console.log(`mergeNewsFromDynamicSource(${name}): inserted ${added}, ignored ${ignored}`);
